	...
	const store = readable(wrapData(items), (set) => {
		if (socketIoEndpoint) {
			...
			const socket = io(socketIoEndpoint, {
				// Would make sense to use extraHeaders, but that is
				// apparently not supported in browsers
				// https://socket.io/docs/v4/client-options/#extraheaders
				// So using query instead: https://socket.io/docs/v4/client-options/#query
				query
			});
			socket.on('connect', () => {
