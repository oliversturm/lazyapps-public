...
<div class="flex flex-col items-center">
  ...
  <div class="flex flex-col md:grid md:grid-cols-2 md:grid-rows-2 gap-4 my-4">
    <AnswerROButton text={format(fffQuestion.answers[0])} marker={answerMarker[0]} order />
    <AnswerROButton text={format(fffQuestion.answers[1])} marker={answerMarker[1]} order />
    <AnswerROButton text={format(fffQuestion.answers[2])} marker={answerMarker[2]} order />
    <AnswerROButton text={format(fffQuestion.answers[3])} marker={answerMarker[3]} order />
  </div>
</div>
    ...
    <table class="results mx-2">
      ...
      {#each viewResults as result, i}
        <tr>
          ...
          >
            {#if adminUI}
              <button on:click={viewResultClick(i)}>{result.playerName}</button
              >
            {:else}{result.playerName}{/if}
          </td
          >
  ...
  <div class="text-center">
    <div class="flex flex-row gap-4 mt-4 justify-center">
      <button
        ...
        disabled={!canPlayOneMoreRound}>Restart Round
      </button
      >
