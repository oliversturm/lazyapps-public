...
const internalJwt = jwt.sign({ user: 'rm-ingame', internal: true }, jwtSecret, {
  issuer: 'millionaire-rm-ingame',
});
...
start({
  readModels: {
    listener: express({
      ...
      // some read models need access for unauthenticated users
      credentialsRequired: false,
    }),
    storage: mongodb({
      url: process.env.MONGO_URL || 'mongodb://127.0.0.1:27017',
      database: process.env.MONGO_DATABASE || 'rm-ingame',
    }),
