...
const log = getLogger('RM/Chng');

const createChangeNotificationHandler = (changeNotificationSender) => {
    ...
    sendChangeNotification: (changeInfo) =>
      queue.add(() =>
        new Promise((resolve) => {
          ...
          .then(changeNotificationSender.sendChangeNotification(changeInfo))
          .catch((err) => {
            log.error(
